...
if length(I) == 2
    % 2D
    plot(X{k}(:,I(1)), X{k}(:,I(2)), 'kx', 'MarkerFaceColor', 'k')  
    plot(feats_ref{k}(I(1)), feats_ref{k}(I(2)), 'go', 'MarkerSize', 10, 'LineWidth', 2)
...
elseif length(I) >= 3
    % 3D
    I = I(1:3); 
    plot3(X{k}(:,I(1)), X{k}(:,I(2)), X{k}(:,I(3)), 'kx', 'MarkerSize', 6)
    plot3(feats_ref{k}(I(1)), feats_ref{k}(I(2)), feats_ref{k}(I(3)), 'go', 'MarkerSize', 10, 'LineWidth', 2)
    xlabel(f_names{I(1)}, 'Interpreter', 'none')
    ylabel(f_names{I(2)}, 'Interpreter', 'none') 
